...
def search_flights(origin: str, destination: str) -> str:
    ...
    result = f"🛩️ Danh sách 4 chuyến bay từ {origin} đến {destination}:\n"
    for f in flights:
        price_str = format_vnd(f['price'])
        result += f"- {f['airline']} | {f['departure']} - {f['arrival']} | Giá: {price_str} ({f['class']})\n"
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = f"🏨 Danh sách khách sạn tại {city} (Dưới {format_vnd(max_price_per_night)}/đêm):\n"
    for h in sorted_hotels:
        result += f"- {h['name']} ({h['stars']}⭐) | Khu vực: {h['area']} | Rating: {h['rating']} | Giá: {format_vnd(h['price_per_night'])}/đêm\n"
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for item in items:
            ...
            name_clean = name.strip().replace('_', ' ').capitalize()
            parsed_expenses[name_clean] = price
            total_cost += price
        ...
        for name, price in parsed_expenses.items():
            result += f"  - {name}: {format_vnd(price)}\n"
        result += "  ---\n"
        result += f"  Tổng chi: {format_vnd(total_cost)}\n"
        ...
        if remaining >= 0:
            ...
            result += f"  Còn lại: -{format_vnd(remaining)}\n"
            result += f"\n⚠️ Vượt ngân sách {format_vnd(remaining)}! Cần điều chỉnh chuyến bay hoặc khách sạn."
            
        return result
        
    except Exception as e:
        return "Lỗi tính toán: Chuỗi chi phí (expenses) không đúng định dạng 'tên:giá,tên:giá'."